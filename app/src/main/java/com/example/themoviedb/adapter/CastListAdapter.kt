...
class CastListAdapter(context: Context, knownFor: ArrayList<Cast>) : RecyclerView.Adapter<CastListAdapter.ViewHolder>() {

    private var context: Context
    private var knownFor: ArrayList<Cast> = ArrayList()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.title.text = knownFor[position].name
        holder.subtitle.text = knownFor[position].character

        val requestOptions = RequestOptions()
        requestOptions.placeholder(R.drawable.load)
        val url = "https://image.tmdb.org/t/p/w500" + knownFor[position].profilePath
